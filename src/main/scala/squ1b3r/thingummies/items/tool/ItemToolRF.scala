 ...
 * ========================================================================
 */
package squ1b3r.thingummies.items.tool
...
import squ1b3r.thingummies.helper.{NBTHelper, StringHelper}
...
trait ItemToolRF extends Item with IEnergyContainerItem {
  ...
  override def addInformation(stack: ItemStack, player: EntityPlayer, list: JList[_], advanced: Boolean): Unit = {
    val tooltip: JList[String] = list.asInstanceOf[JList[String]]
  ...
  protected def updateInformation(stack: ItemStack, player: EntityPlayer, tooltip: JList[String]): Unit = {}

  @SideOnly(Side.CLIENT)
  override def getSubItems(item: Item, tab: CreativeTabs, list: JList[_]): Unit = {
    list.asInstanceOf[JList[ItemStack]].add(NBTHelper.writeToNBT(new ItemStack(item, 1, 0), "Energy", 0))
    list.asInstanceOf[JList[ItemStack]].add(NBTHelper.writeToNBT(new ItemStack(item, 1, 0), "Energy", maxEnergy))
  }
